...
const initialState = {
  ...
  LocationImgForService:Constants.Images.customer.pickup,
  ...
  TabHeaderImages:[
    {tag:0,displayimg:Constants.Images.customer.locationmenu,img:Constants.Images.customer.locationmenu,activeimg:Constants.Images.customer.locationmenu_active,color:Constants.Colors.LightBlue},
    {tag:1,displayimg:Constants.Images.customer.servicesmenu,img:Constants.Images.customer.servicesmenu,activeimg:Constants.Images.customer.servicesmenu_active,color:Constants.Colors.LightBlue},
    {tag:2,displayimg:Constants.Images.customer.paymentmenu,img:Constants.Images.customer.paymentmenu,activeimg:Constants.Images.customer.paymentmenu_active,color:Constants.Colors.LightBlue},
    {tag:3,displayimg:Constants.Images.customer.drivermenu,img:Constants.Images.customer.drivermenu,activeimg:Constants.Images.customer.drivermenu_active,color:Constants.Colors.LightBlue},
  ],


  HourlyServiceTabHeaderImages:[
    {tag:0,displayimg:Constants.Images.customer.timemenu,img:Constants.Images.customer.timemenu,activeimg:Constants.Images.customer.timemenu_active,color:Constants.Colors.LightBlue},
    {tag:1,displayimg:Constants.Images.customer.hourlylocationmenu,img:Constants.Images.customer.hourlylocationmenu,activeimg:Constants.Images.customer.hourlylocationmenu_active,color:Constants.Colors.LightBlue},
    {tag:2,displayimg:Constants.Images.customer.paymentmenu,img:Constants.Images.customer.paymentmenu,activeimg:Constants.Images.customer.paymentmenu_active,color:Constants.Colors.LightBlue},
    {tag:3,displayimg:Constants.Images.customer.drivermenu,img:Constants.Images.customer.drivermenu,activeimg:Constants.Images.customer.drivermenu_active,color:Constants.Colors.LightBlue},
  ],
  ...
  markerPositions:[
    {id:0,title :'You are here11', coordinates:{latitude:28.6139,longitude:77.2090,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup},
  ],
  DriverPositions:[{
    latitude:28.6139,
    longitude:77.2090,
...
export default function CustomerReducer(state = initialState, action = {})
{
    ...
    switch (action.type) {
        ...
        if(newstate.DeliveryFlag==1)
        {
          ...
          if(newstate.pickupArr.length>1)
          {
            newstate.pickupArr.map((addressVal,i) => {
              ...
              {
                arrMarker.push({id:addressVal.id,pickdropflag:1,pickdropId:i,title :'Pickup '+(i + 1),coordinates:{latitude:addressVal.lat,longitude:addressVal.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup});
              }
            });
          }
          if(newstate.dropArr.length>1)
          {
            newstate.dropArr.map((addressVal,i) => {
              if(!(addressVal.address.indexOf('Choose')==0))
              {
                arrMarker.push({id:addressVal.id,pickdropflag:2,pickdropId:i,title :'Drop '+(i + 1),coordinates:{latitude:addressVal.lat,longitude:addressVal.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.drop});
              }
            });
          }
        ...
        newstate.TransportArr.map((val,i) => {
          if(parseInt(newstate.vehicleID) == parseInt(val.tag))
          {
            ...
            arr1.push({tag:val.tag,displayimg:val.activeimgsource,imgsource:val.imgsource,
              ...
              backgroundColor: newstate.EstimateButtonBackgroundColor,borderBottomWidth:4,borderBottomColor:Constants.Colors.LightBlue,cost:val.cost,name:val.name});
          }
          else {
      ...
      case 'ADD_PICKUP':
        ...
        {
          arrMarker.push({id:0,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Pickup '+(len), coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup});
        }
        else {
          arrMarker.push({id:markerlen,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Pickup '+(len), coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup});
        }
        ...
        newstate.TransportArr.map((val,i) => {
          if(parseInt(newstate.vehicleID) == parseInt(val.tag))
            arr1.push({tag:val.tag,displayimg:val.activeimgsource,imgsource:val.imgsource,
              activeimgsource:val.activeimgsource,header:val.header,
              backgroundColor: newstate.EstimateButtonBackgroundColor,borderBottomWidth:4,borderBottomColor:Constants.Colors.LightBlue,cost:val.cost});
          else {
            arr1.push({tag:val.tag,displayimg:val.imgsource,imgsource:val.imgsource,
              activeimgsource:val.activeimgsource,header:val.header,
      ...
      case 'ADD_DROP':
        ...
        if(newstate.markerPositions[0].title.indexOf('You are here')==0)
        {
          arrMarker.push({id:0,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Drop '+(len), coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.drop});
        }
        else
        {
          arrMarker.push({id:markerlen,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Drop '+(len), coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.drop});
        }
        ...
        if(newstate.HourlyServiceCount==3)
        {
          ...
          newstate.HourlyServiceTabHeaderImages.map((val,i) => {
            if(parseInt(val.tag) == 0)
              arr1.push({tag:val.tag,displayimg:val.activeimg,img:val.img,
                activeimg:val.activeimg,color:Constants.Colors.White});
            else {
              arr1.push({tag:val.tag,displayimg:val.img,img:val.img,
                activeimg:val.activeimg,color:Constants.Colors.LightBlue});
            }
          });
      ...
      case 'SET_FOODRANGE':
        ...
        if(newstate.pickupArr.length>=newstate.dropArr.length)
        {
          newstate.pickupArr.map((val,i) => {
            if(!(val.address.indexOf('Choose')==0))
              arr1.push({id:val.id,address : val.address,img : val.img,next:action.range,prev:1,lat:val.lat,long:val.long,
              opacity:1,activecolor:'#C3C1C0',});
          });
          newstate.LocationForService='Pickup ';
          newstate.LocationImgForService = Constants.Images.customer.pickup;
        }
        else {
          newstate.dropArr.map((val,i) => {
            if(!(val.address.indexOf('Choose')==0))
          ...
          newstate.LocationImgForService = Constants.Images.customer.drop;
        }
      ...
      case 'SET_ITEMRANGE_ARRAY':
        ...
        var temp=action.array;
          newstate.DisplayLocationAddress.map((val,i) => {
              arr1.push({id:val.id,address : val.address,img : val.img, next:action.array[i].next,prev:action.array[i].prev,lat:val.lat,long:val.long,
        ...
        newstate.TransportArr.map((val,i) => {
          for(var i=0;i<transportArr1.length;i++)
          {
            if(parseInt(val.tag)==transportArr1[i].id)
            {
              arr1.push({tag:val.tag,displayimg:val.imgsource,imgsource:val.imgsource,
                activeimgsource:val.activeimgsource,header:val.header,
                backgroundColor:'transparent',borderBottomWidth:0,borderBottomColor:'transparent',cost:'$'+transportArr1[i].charge,name:transportArr1[i].name});
            }
          }
        });
      ...
      case 'ADD_PICKUP_HOURLY':
        ...
        newstate.markerPositions.map((val,i) => {
          ...
          arrMarker.push({id:0,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Pickup', coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup});
      ...
      case 'ADD_DROP_HOURLY':
        ...
        newstate.markerPositions.map((val,i) => {
          ...
          arrMarker.push({id:1,pickdropflag:newstate.PickDropFlag,pickdropId:len-1,title :'Drop', coordinates:{latitude:action.lat,longitude:action.long,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.drop});
      ...
      case 'SET_INITIALLOCATION':
        ...
        var arr1=[];
        arr1.push({id:-1,pickdropflag:newstate.PickDropFlag,pickdropId:-1,title :'You are here', coordinates:{latitude:action.markerPosition.latitude,longitude:action.markerPosition.longitude,latitudeDelta: 1,longitudeDelta: 1},img:Constants.Images.customer.pickup});
        newstate.markerPositions=[];
      ...
      case 'COURIER_ITEMADD':
        ...
        newstate.DisplayLocationAddress.map((val,b) => {
          if(parseInt(newstate.CourierItemIndex)==parseInt(val.id))
          {
            ...
            {
              _courieritems.push({parentId:newstate.CourierItemIndex,id:val.courieritems[i].id,name:val.courieritems[i].name,
        ...
        newstate.DisplayLocationAddress.map((val,b) => {
          if(parseInt(_parentid)==parseInt(val.id))
          {
            for(var i=0;i<val.courieritems.length;i++)
            {
              ...
              {
                _courieritems.push({parentId:val.courieritems[i].parentId,id:val.courieritems[i].id,name:val.courieritems[i].name,
      ...
      case 'FURNITURE_MODAL_NEWADD':
        ...
        newstate.FurnitureItemList.map((obj,a) => {
          ...
          {
            _qty=0;
            products.push({category:obj.products[j].category,id:obj.products[j].id,
        ...
        newstate.DisplayLocationAddress.map((val,b) => {
          if(parseInt(_parentid)==parseInt(val.id))
          {
            for(var i=0;i<val.furnitureitems.length;i++)
            {
              if(val.furnitureitems[i].id == _id)
              {
                furnitureItems.push({parentId:val.furnitureitems[i].parentId,category:val.furnitureitems[i].category,id:val.furnitureitems[i].id,
