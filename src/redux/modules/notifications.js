...
export const getNotificationList   = (data) => ({ type: GET_ALL_NOTIFICATIONS , data});
...
export const getAllNotification = (token) =>{
  ...
  return dispatch => {
    ...
    RestClient.get("notifications", {}, token).then((result) => {
      
      if(result.status==1){
        console.log("notificatons lists *********** inside noti",result)
    		dispatch(stopLoading());
        dispatch(ToastActionsCreators.displayInfo(result.message));	
        dispatch(getNotificationList(result.data));
